...
int pendingCallUdpPort = 0; // Added
int activeCallUdpPort = 0;  // Added
...
void printColoredNoLock(const std::string &text,
...
void handlePushNotification(const std::string &message) {
  ...
  if (messageType == "RECEIVE_MESSAGE") {
    ...
    if (isChattingWithSender) {
      ...
      if (canShowNotification) {
        ...
        std::cout
            << "\033[33m╔══════════════════════════════════════════╗\033[0m\n";
        std::cout << "\033[33m║  📬 NEW MESSAGE from \033[36m" << senderName
                  << "\033[33m\033[0m\n";
        std::cout << "\033[33m║  \033[0m\"" << messageContent << "\"\n";
        std::cout
            << "\033[33m║  Press 'r' in menu to reply quickly      ║\033[0m\n";
        std::cout
            << "\033[33m╚══════════════════════════════════════════╝\033[0m\n";
        std::cout << std::flush;
      }
    ...
    if (!messages.empty()) {
      ...
      if (canShowNotification) {
        ...
        std::cout
            << "\033[33m╔══════════════════════════════════════════╗\033[0m\n";
        std::cout << "\033[33m║  📬 You have \033[36m" << unreadCount
                  << "\033[33m unread message(s)!          ║\033[0m\n";
        std::cout
            << "\033[33m╠══════════════════════════════════════════╣\033[0m\n";
        ...
        for (const std::string &msg : messages) {
          ...
          std::string preview =
              content.length() > 25 ? content.substr(0, 25) + "..." : content;
          std::cout << "\033[33m║  \033[36m" << senderName
                    << "\033[0m: " << preview << "\n";
        }
        ...
        std::cout
            << "\033[33m╠══════════════════════════════════════════╣\033[0m\n";
        std::cout
            << "\033[33m║  Go to Chat (option 4) to reply!         ║\033[0m\n";
        std::cout
            << "\033[33m╚══════════════════════════════════════════╝\033[0m\n";
        std::cout << std::flush;
      }
  ...
  // Voice Call notifications
...

    {
      ...
      std::string portStr = getJsonValue(payload, "udpPort");
      if (!portStr.empty())
        pendingCallUdpPort = std::stoi(portStr);
      else
        pendingCallUdpPort = 0;
    ...
    if (canShowNotification && !inCallMode) {
      ...
      std::cout
          << "\033[35m╔══════════════════════════════════════════╗\033[0m\n";
      std::cout << "\033[35m║  📞 INCOMING CALL from \033[36m" << callerName
                << "\033[35m       ║\033[0m\n";
      std::cout
          << "\033[35m║  Press 'c' in menu to answer/reject      ║\033[0m\n";
      std::cout
          << "\033[35m╚══════════════════════════════════════════╝\033[0m\n";
      std::cout << std::flush;
#endif
...

    {
      ...
      std::string portStr = getJsonValue(payload, "udpPort");
      if (!portStr.empty())
        activeCallUdpPort = std::stoi(portStr);
      else
        activeCallUdpPort = 0;
    }
    ...
    std::cout
        << "\033[32m╔══════════════════════════════════════════╗\033[0m\n";
    std::cout << "\033[32m║  CALL CONNECTED with \033[36m" << receiverName
              << "\033[32m!\033[0m\n";
    std::cout
        << "\033[32m║  (Simulated voice call active)           ║\033[0m\n";
    std::cout
        << "\033[32m╚══════════════════════════════════════════╝\033[0m\n";
    std::cout << std::flush;
    ...
    std::cout
        << "\033[31m╔══════════════════════════════════════════╗\033[0m\n";
    std::cout
        << "\033[31m║  CALL REJECTED by receiver               ║\033[0m\n";
    std::cout
        << "\033[31m╚══════════════════════════════════════════╝\033[0m\n";
    std::cout << std::flush;
    ...
    std::cout
        << "\033[33m╔══════════════════════════════════════════╗\033[0m\n";
    std::cout << "\033[33m║  CALL ENDED (Duration: " << durationStr
              << "s)\033[0m\n";
    std::cout
        << "\033[33m╚══════════════════════════════════════════╝\033[0m\n";
    std::cout << std::flush;
  ...
  while (running && clientSocket >= 0) {
    ...
    if (pfd.revents & POLLIN) {
      ...
      while (totalRead < msgLen) {
        ssize_t n =
            recv(clientSocket, &buffer[totalRead], msgLen - totalRead, 0);
        if (n <= 0) {
          if (running) {
            running = false;
          }
...
bool sendMessage(const std::string &message) {
  ...
  while (totalSent < message.length()) {
    ssize_t sent = send(clientSocket, message.c_str() + totalSent,
                        message.length() - totalSent, 0);
    if (sent <= 0)
      return false;
    totalSent += sent;
  }
...
// Forward declaration
void openChatWith(const std::string &recipientId,
...
bool registerAccount() {
  ...
  std::string request =
      R"({"messageType":"REGISTER_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"payload":{"fullname":")" +
      escapeJson(fullname) + R"(","email":")" + escapeJson(email) +
      R"(","password":")" + escapeJson(password) + R"(","confirmPassword":")" +
      escapeJson(confirmPassword) + R"("}})";
...
bool login() {
  ...
  std::string request = R"({"messageType":"LOGIN_REQUEST","messageId":")" +
                        generateMessageId() + R"(","timestamp":)" +
...
void setLevel() {
  ...
  printColored("\nEnter level (1/2/3 or beginner/intermediate/advanced): ",
               "green");
  ...
  std::string request = R"({"messageType":"SET_LEVEL_REQUEST","messageId":")" +
                        generateMessageId() + R"(","timestamp":)" +
                        std::to_string(getCurrentTimestamp()) +
...
void viewLessons() {
  while (true) {
    ...
    std::string request =
        R"({"messageType":"GET_LESSONS_REQUEST","messageId":")" +
        generateMessageId() + R"(","timestamp":)" +
        std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
        sessionToken + R"(","payload":{"level":"","topic":")" + topic +
        R"(","page":1,"limit":20}})";
    ...
    for (const std::string &lesson : lessonsList) {
      ...
      printf("│ %-2d │ %-11s │ %-34s │ %-12s │ %-8s │\n", idx, lessonId.c_str(),
             title.c_str(), lessonTopic.c_str(), level.c_str());
      idx++;
    }
    ...
    printColored("\nOptions:\n", "yellow");
    printColored("  Enter lesson number (1-" +
                     std::to_string(lessonsList.size()) + ") to study\n",
                 "");
    ...
    try {
      ...
      if (lessonNum >= 1 && lessonNum <= (int)lessonIds.size()) {
        ...
        std::string detailRequest =
            R"({"messageType":"GET_LESSON_DETAIL_REQUEST","messageId":")" +
            generateMessageId() + R"(","timestamp":)" +
            std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
            sessionToken + R"(","payload":{"lessonId":")" + selectedLessonId +
            R"("}})";
        ...
        printColored("\n═══════════════════════════════════════════════════════"
                     ...
                     "cyan");
        printColored(
            "[Video/Audio content would be displayed here in a real app]\n",
            "magenta");
        printColored("═════════════════════════════════════════════════════════"
                     "═══════════════════════\n",
                     "cyan");
...
void takeTest() {
  ...
  // Lấy đề test
  std::string request = R"({"messageType":"GET_TEST_REQUEST","messageId":")" +
                        generateMessageId() + R"(","timestamp":)" +
                        std::to_string(getCurrentTimestamp()) +
                        R"(,"sessionToken":")" + sessionToken +
                        R"(","payload":{"testType":"mixed","level":")" +
                        currentLevel + R"(","topic":"grammar"}})";
  ...
  for (size_t i = 0; i < questions.size(); i++) {
    ...
    printColored("────────────────────────────────────────────\n", "cyan");
    printColored("Question " + std::to_string(i + 1) + "/" +
                     std::to_string(questions.size()) + " (" + points +
                     " points)\n",
                 "magenta");
    ...
    if (type == "multiple_choice") {
      ...
      printColored("\nType the sentence in correct order (separate words with "
                   "spaces):\n",
                   "yellow");
  ...
  for (size_t i = 0; i < answers.size(); i++) {
    if (i > 0)
      answersJson << ",";
    answersJson << R"({"questionId":")" << answers[i].first << R"(","answer":")"
                << escapeJson(answers[i].second) << R"("})";
  }
  answersJson << "]";

  std::string submitRequest =
      R"({"messageType":"SUBMIT_TEST_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"testId":")" + testId +
  ...
  printColored("                        │\n", "cyan");
  printColored("│  Grade: ", "");
  printColored(grade, (grade == "A" || grade == "B")   ? "green"
...
void doExercises() {
  ...
  // Get exercise
  std::string request =
      R"({"messageType":"GET_EXERCISE_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"exerciseType":")" + exerciseType +
  ...
  if (exerciseType == "sentence_rewrite") {
    ...
    printColored("\n────────────────────────────────────────────\n", "cyan");
    printColored("Type your rewritten sentences (one per line, press Enter "
                 "twice when done):\n",
                 "yellow");
    ...
    while (std::getline(std::cin, line)) {
      if (line.empty() && !rewrittenSentences.empty())
        break;
      if (!line.empty()) {
  ...
  std::string submitRequest =
      R"({"messageType":"SUBMIT_EXERCISE_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"exerciseId":")" + exerciseId +
      R"(","exerciseType":")" + exerciseType + R"(","content":")" +
      escapeJson(content) + R"("}})";
...
void openChatWith(const std::string &recipientId,
  ...
  // Lấy lịch sử chat
  std::string historyRequest =
      R"({"messageType":"GET_CHAT_HISTORY_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"recipientId":")" + recipientId + R"("}})";
  ...
  std::string markReadRequest =
      R"({"messageType":"MARK_MESSAGES_READ_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"senderId":")" + recipientId + R"("}})";
  ...
  while (inChatMode && running) {
    ...
    std::string chatRequest =
        R"({"messageType":"SEND_MESSAGE_REQUEST","messageId":")" +
        generateMessageId() + R"(","timestamp":)" +
        std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
        sessionToken + R"(","payload":{"recipientId":")" + recipientId +
...
void viewFeedback() {
  ...
  std::string request =
      R"({"messageType":"GET_USER_SUBMISSIONS_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{}})";
  ...
  printColored("\n══════════ SUMMARY ══════════\n", "magenta");
  printColored(
      "Total Submissions: " + std::to_string(submissions.size()) + "\n", "");
...
void playGames() {
  ...
  // Get game list
  std::string request =
      R"({"messageType":"GET_GAME_LIST_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"gameType":"all","level":")" +
      currentLevel + R"("}})";
  ...
  std::string startRequest =
      R"({"messageType":"START_GAME_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"gameId":")" + selectedGameId + R"("}})";
  ...
  if (gameType == "word_match") {
    ...
    printColored("\nEnter matches (format: left1-right1,left2-right2,...):\n",
                 "yellow");
    printColored("Example: 1-3,2-1,3-5 means: English1-Vietnamese3, "
                 "English2-Vietnamese1, etc.\n",
                 "cyan");
    ...
    while (std::getline(iss, match, ',')) {
      ...
      if (dashPos != std::string::npos) {
        ...
        if (leftIdx >= 0 && leftIdx < (int)leftItems.size() && rightIdx >= 0 &&
            rightIdx < (int)rightItems.size()) {
          if (!first)
            matchesJson << ",";
          first = false;
          matchesJson << R"({"left":")" << escapeJson(leftItems[leftIdx])
                      << R"(","right":")" << escapeJson(rightItems[rightIdx])
                      << R"("})";
        }
      }
    }
    ...
    // Submit result
    std::string submitRequest =
        R"({"messageType":"SUBMIT_GAME_RESULT_REQUEST","messageId":")" +
        generateMessageId() + R"(","timestamp":)" +
        std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
        sessionToken + R"(","payload":{"gameSessionId":")" + gameSessionId +
        R"(","gameId":")" + selectedGameId + R"(","matches":)" +
        matchesJson.str() + R"(}})";
    ...
    if (submitStatus == "success") {
      ...
      printColored(
          "Correct Matches: " + correctMatches + "/" + totalPairs + "\n", "");
    ...
    for (size_t i = 0; i < words.size(); i++) {
      ...
      snprintf(buffer, sizeof(buffer), "│ %zu. %-20s │ %zu. %-20s │\n", i + 1,
               words[i].c_str(), i + 1, emojis[i].c_str());
      printColored(buffer, "");
    }
    printColored("└─────────────────────────┴─────────────────────────┘\n\n",
                 "cyan");

    printColored("\nEnter matches (format: word1-img1,word2-img2,...): ",
                 "green");
    std::string matchesInput;
    std::getline(std::cin, matchesInput);
    ...
    while (std::getline(iss, match, ',')) {
      ...
      if (dashPos != std::string::npos) {
        ...
        if (wordIdx >= 0 && wordIdx < (int)words.size() && imgIdx >= 0 &&
            imgIdx < (int)imageUrls.size()) {
          if (!first)
            matchesJson << ",";
          first = false;
          matchesJson << R"({"word":")" << escapeJson(words[wordIdx])
                      << R"(","imageUrl":")" << escapeJson(imageUrls[imgIdx])
                      << R"("})";
        }
      }
    }
...
void chat() {
  ...
  if (!quickReplyId.empty()) {
    ...
    printColored("\n", "");
    printColored(
        "Press 'r' to reply directly, or any other key to see contact list: ",
        "green");
  ...
  std::string request =
      R"({"messageType":"GET_CONTACT_LIST_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"contactType":"all","online":false}})";
...
void answerIncomingCall() {
  ...
  if (input == "1") {
    // Accept call
    std::string request =
        R"({"messageType":"VOICE_CALL_ACCEPT_REQUEST","messageId":")" +
        generateMessageId() + R"(","timestamp":)" +
        std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
        sessionToken + R"(","payload":{"callId":")" + callId + R"("}})";
    ...
    if (status == "success") {
      ...
      std::string endRequest =
          R"({"messageType":"VOICE_CALL_END_REQUEST","messageId":")" +
          generateMessageId() + R"(","timestamp":)" +
          std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
          sessionToken + R"(","payload":{"callId":")" + callId + R"("}})";
    ...
    // Reject call
    std::string request =
        R"({"messageType":"VOICE_CALL_REJECT_REQUEST","messageId":")" +
        generateMessageId() + R"(","timestamp":)" +
        std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
        sessionToken + R"(","payload":{"callId":")" + callId + R"("}})";
...
void voiceCall() {
  ...
  // Check for incoming call first
  {
    ...
    if (hasIncomingCall && !pendingCallId.empty()) {
      ...
      printColored(pendingCallerName, "cyan");
      printColored("!\n", "yellow");
      printColored("Press 'a' to answer, or any other key to make a new call: ",
                   "green");
  ...
  std::string request =
      ...
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"contactType":"all","online":true}})";
  ...
  std::string callRequest =
      R"({"messageType":"VOICE_CALL_INITIATE_REQUEST","messageId":")" +
      generateMessageId() + R"(","timestamp":)" +
      std::to_string(getCurrentTimestamp()) + R"(,"sessionToken":")" +
      sessionToken + R"(","payload":{"receiverId":")" + receiverId +
...
#ifndef CLIENT_SKIP_MAIN
int main(int argc, char *argv[]) { return main_cli(argc, argv); }
#endif